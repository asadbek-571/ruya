...
public class ResAnnouncementOne implements Serializable {
    ...
    public ResAnnouncementOne(Announcement ann) {
        ...
        if (CoreUtils.isPresent(ann.getDetails()) && CoreUtils.isPresent(ann.getDetails().getPrice())) {
            AnnouncementDetails details = ann.getDetails();
            this.amount = new ResAmount(details.getPrice(), details.getCurrency());
        }
    }
}
